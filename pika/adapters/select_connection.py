...
class PollEvents(object):
    ...
    READ = getattr(select, 'POLLIN', 0) or 0x01 # available for read
    WRITE = getattr(select, 'POLLOUT', 0) or 0x04  # available for write
    ERROR = getattr(select, 'POLLERR', 0) or 0x08 # error on associated fd
